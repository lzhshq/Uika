...
bool robot_msgs__msg__robot_command__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // motor_command
    ...
    if (!robot_msgs__msg__MotorCommand__Sequence__init(&(ros_message->motor_command), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create robot_msgs__msg__MotorCommand__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    robot_msgs__msg__MotorCommand * dest = ros_message->motor_command.data;
...
PyObject * robot_msgs__msg__robot_command__convert_to_py(void * raw_ros_message)
{
  ...
  {  // motor_command
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->motor_command.data[i]);
