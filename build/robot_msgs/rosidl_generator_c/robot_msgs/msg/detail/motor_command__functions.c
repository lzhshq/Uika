  ...
  robot_msgs__msg__MotorCommand__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    robot_msgs__msg__MotorCommand * data =
      (robot_msgs__msg__MotorCommand *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!robot_msgs__msg__MotorCommand__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          robot_msgs__msg__MotorCommand__fini(&output->data[i]);
        }
        return false;
      }
    }
