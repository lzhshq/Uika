...
说明：
  ...
  - 然后用 smoothstep 插值到 12 个电机的 0 位置。
...
# 零位置，单位 rad。顺序必须和 JOINT_FIELDS 一致。
ZERO_POSITIONS = [0.0] * len(JOINT_FIELDS)
...
class SoftStartZero(Node):
    ...
    def make_command(self, positions):
        ...
        msg.header.frame_id = 'soft_start_zero'
    ...
    def run(self) -> bool:
        ...
        if not self.wait_feedback(timeout=5.0):
            self.get_logger().error('5 秒内没有收到 /motor_feedback，取消软回零。')
            return False

        start = self.feedback_positions()
        target = ZERO_POSITIONS
        max_delta = max(abs(b - a) for a, b in zip(start, target))
        self.get_logger().info(
            f'开始软动作到 0 位置: duration={self.duration:.2f}s, '
        ...
        while rclpy.ok() and time.monotonic() < hold_end:
            ...
            time.sleep(period)
        self.pub.publish(final_msg)
        self.get_logger().info('软回零完成，已发布 0 位置。')
        return True
...
def main():
    ...
    rclpy.init()
    node = SoftStartZero(args.duration, args.rate, args.hold_time)
    try:
        ok = node.run()
