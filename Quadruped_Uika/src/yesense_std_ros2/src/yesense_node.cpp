...
quat_xyzw_t euler_rpy_to_quat(double roll, double pitch, double yaw)
{
...
bool normalize_quat(quat_xyzw_t *quat)
{
	if(NULL == quat)
	{
		return false;
...
class YESENSE_Publisher : public rclcpp::Node
{
	...
	rclcpp::Publisher<geometry_msgs::msg::Vector3Stamped>::SharedPtr pub_rpy_geometry;
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result)
{
	...
	{
		quat.x = result->quat.q1;
		quat.y = result->quat.q2;
		quat.z = result->quat.q3;
		quat.w = result->quat.q0;
	}
	else if(result->content.euler)
	{
		quat = euler_rpy_to_quat(
			result->euler.roll / DEG_TO_RAD_FACTOR,
			result->euler.pitch / DEG_TO_RAD_FACTOR,
			result->euler.yaw / DEG_TO_RAD_FACTOR);
	}
	else
	{
		quat.x = 0.0;
	...
	if(result->content.quat || result->content.euler)
	{
		imu_ros_data.orientation_covariance[0] = 0.02;
		imu_ros_data.orientation_covariance[4] = 0.02;
		imu_ros_data.orientation_covariance[8] = 0.02;
	}
	else
	{
	...
	if(result->content.gyro)
	{
		...
		for(int i = 0; i < 9; i++)
		{
			imu_ros_data.angular_velocity_covariance[i] = 0.02;
		}
	}
	else
	{
		imu_ros_data.angular_velocity_covariance[0] = -1.0;
	}
	...
	if(result->content.acc)
	{
		...
		for(int i = 0; i < 9; i++)
		{
			imu_ros_data.linear_acceleration_covariance[i] = 0.02;
		}
	}
	else
	{
		imu_ros_data.linear_acceleration_covariance[0] = -1.0;
	}
	...
	if(publish_rpy && result->content.euler)
	{
		...
		rpy_data.vector.y = result->euler.pitch;
		rpy_data.vector.z = result->euler.yaw;
