...
class DmImuNode(Node):
    ...
    def _on_timer_publish(self):
        try:
            imu_data, stamp_ts, count = self.ser.get_latest()
        except Exception as e:
            if self.verbose:
        ...
        # 检查是否有任何有效数据
        if not imu_data.rpy_valid and not imu_data.acc_valid and not imu_data.gyro_valid:
            return
        ...
        # /imu/rpy
        if self.pub_rpy is not None and imu_data.rpy_valid:
            r_deg, p_deg, y_deg = imu_data.roll, imu_data.pitch, imu_data.yaw
        ...
        if self.pub_imu is not None:
            ...
            if imu_data.gyro_valid:
                imu_msg.angular_velocity.x = imu_data.gyro_x * math.pi / 180.0
                imu_msg.angular_velocity.y = imu_data.gyro_y * math.pi / 180.0
                imu_msg.angular_velocity.z = imu_data.gyro_z * math.pi / 180.0
                for i in range(9):
            ...
            # 线加速度 (m/s²)
            if imu_data.acc_valid:
                imu_msg.linear_acceleration.x = imu_data.acc_x
                imu_msg.linear_acceleration.y = imu_data.acc_y
                imu_msg.linear_acceleration.z = imu_data.acc_z
                for i in range(9):
                    imu_msg.linear_acceleration_covariance[i] = 0.02
            else:
                for i in range(9):
        ...
        if self.verbose:
            if imu_data.rpy_valid:
                self.get_logger().info(
                    f'#{self._pub_count} RPY(deg)=({imu_data.roll:.2f}, {imu_data.pitch:.2f}, {imu_data.yaw:.2f}) '
                    f'Gyro(°/s)=({imu_data.gyro_x:.2f}, {imu_data.gyro_y:.2f}, {imu_data.gyro_z:.2f}) '
                    f'Acc(m/s²)=({imu_data.acc_x:.2f}, {imu_data.acc_y:.2f}, {imu_data.acc_z:.2f})'
                )
