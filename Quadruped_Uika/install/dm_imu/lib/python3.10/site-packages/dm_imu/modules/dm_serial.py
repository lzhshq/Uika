...
# 帧类型ID定义
FRAME_ID_ACC = 0x01   # 加速度
FRAME_ID_GYRO = 0x02  # 角速度
FRAME_ID_RPY = 0x03   # 姿态角


class ImuData:
    """统一 IMU 数据结构，存储加速度、角速度、姿态角"""
    def __init__(self):
        # 加速度 (m/s²)
        self.acc_x = 0.0
        self.acc_y = 0.0
...
class DM_Serial:
    ...
    # ------------ 公共 API ------------
    def read(self, max_bytes: int | None = None) -> Optional[Dict[str, Any]]:
        """
        一次性读入串口当前可读字节，解析所有完整帧，更新 ImuData。
        返回是否有任何有效数据。
        """
        ...
        with self._latest_lock:
            for frame in frames:
                rid, data = frame
                if rid == FRAME_ID_ACC:
                    self._imu_data.acc_x, self._imu_data.acc_y, self._imu_data.acc_z = data
                    self._imu_data.acc_valid = True
                elif rid == FRAME_ID_GYRO:
                    self._imu_data.gyro_x, self._imu_data.gyro_y, self._imu_data.gyro_z = data
                    self._imu_data.gyro_valid = True
                elif rid == FRAME_ID_RPY:
                    self._imu_data.roll, self._imu_data.pitch, self._imu_data.yaw = data
                    self._imu_data.rpy_valid = True

        return self._imu_data if frames else None
    ...
    def get_latest(self) -> Tuple[ImuData, float, int]:
        """线程安全地获取（imu_data, timestamp, count）。"""
        with self._latest_lock:
            return self._imu_data, self._latest_ts, self._latest_count
    ...
    def _reader_loop(self):
        ...
        try:
            while evt and not evt.is_set():
                self.read(None)  # read() 会更新 _imu_data
                with self._latest_lock:
                    if self._imu_data.acc_valid or self._imu_data.gyro_valid or self._imu_data.rpy_valid:
