 ...
 * @param cloudIn
 * @param cloudOut
 * @param rotation_matrix 外参旋转矩阵
 * @cloud_filter_box 将旋转后的点云在box中的部分滤除
 */
void transformUnitreeCloudToPCL(
  const PointCloudUnitree& cloudIn,
  pcl::PointCloud<PointType>::Ptr cloudOut,
  Eigen::Matrix3d &rotation_matrix,
  CloudFilterBox &cloud_filter_box) {
    ...
    for (size_t i = 0; i < cloudIn.points.size(); i ++) {
      ...
      Eigen::Vector3d rotatedPoint = rotation_matrix * point;

      // Self-filtering
      if (!cloud_filter_box.IsPointValid(rotatedPoint.x(), rotatedPoint.y(), rotatedPoint.z())) {
        continue;
      }
